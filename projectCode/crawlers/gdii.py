...
class GDIIcrawler(BaseResearchCrawler):
    """广东省工业和信息化厅爬虫"""
    ...
    def fetch_notice_list(self, list_url: str, max_pages: int = 3) -> List[Dict[str, str]]:
        ...
        try:
            ...
            while page_num <= max_pages:
                ...
                # 结构1: ul > li
                ul_list = soup.find("ul", class_=lambda x: x and any(keyword in str(x) for keyword in ["list", "notice", "news"]))
                if ul_list:
                ...
                if not items:
                    # 结构3: 查找所有带链接的div
                    items = []
                    for div in soup.find_all("div", class_=lambda x: x and "item" in str(x).lower()):
    ...
    def fetch_notice_detail(self, detail_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if content_div:
                ...
                text = "\n".join(
                    [p.get_text(strip=True) for p in paragraphs if len(p.get_text(strip=True)) > 3]
                )
